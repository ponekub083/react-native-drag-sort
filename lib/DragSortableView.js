...
export default class DragSortableView extends Component{
    ...
    constructor(props) {
        ...
        this._panResponder = PanResponder.create({
            ...
            onStartShouldSetPanResponderCapture: (evt, gestureState) => {
                this.startClick(evt.nativeEvent.pageX)
    ...
    startClick (positionX) {
      if(this.props.firstClick)
        this.props.firstClick(positionX);
    }
    ...
    moveTouch (nativeEvent,gestureState) {
        ...
        if (this.touchCurItem) {
            ...
            const maxWidth = itemWidth * this.state.dataSource.length;
            ...
            if (!this.props.isDragFreely) {
                ...
                if (this.touchCurItem.originLeft + dx < 0) {
                    dx = -this.touchCurItem.originLeft + this.props.verticalOffset
                } else if (this.touchCurItem.originLeft + dx > maxWidth) {
                    dx = maxWidth - this.touchCurItem.originLeft + this.props.verticalOffset
                }
            ...
            console.log( this.props.updateOffset())
            const offset = this.props.updateOffset();
            let left = this.touchCurItem.originLeft + dx + offset;
            ...
            let moveToIndex = 0
            let moveXNum = left/itemWidth
            let moveYNum = top/itemHeight
            if (moveXNum > 0) {
        ...
        return (
            <View
                ...
                style={[styles.container,{
                    flex : 1,
                    width: this.state.itemWidth * this.state.dataSource.length,
    ...
    _getKey = (index) => {
        const item = this.state.dataSource[index].data.attributes();
        return this.props.keyExtractor ? this.props.keyExtractor(item , index) : item.originIndex;
    }
